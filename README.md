...
│   ├── VlnPlot_Tcell_markers.png             # Fig 9 — Violin plots: T cell subtype markers across conditions
│   ├── VlnPlot_TopDE_RM.png                  # Fig 10 — Violin plots: top upregulated DE genes, RM
│   ├── VlnPlot_TopDE_LNG.png                 # Fig 11 — Violin plots: top upregulated DE genes, LNG
│   ├── GSEA_GO_dotplot_RM.png                # Fig 12 — GO BP GSEA dotplot, RM
│   ├── GSEA_GO_dotplot_OM.png                # Fig 13 — GO BP GSEA dotplot, OM
│   ├── GSEA_GO_dotplot_LNG.png               # Fig 14 — GO BP GSEA dotplot, LNG
│   ├── GSEA_Hallmark_dotplot_RM.png          # Fig 15 — Hallmark GSEA dotplot, RM
│   ├── GSEA_Hallmark_dotplot_OM.png          # Fig 16 — Hallmark GSEA dotplot, OM
...
│   ├── GSEA_Hallmark_Tcells_D08vsNaive_RM.csv
│   ├── GSEA_Hallmark_Tcells_D08vsNaive_OM.csv
│   └── GSEA_Hallmark_Tcells_D08vsNaive_LNG.csv
│
...
Influenza A Virus (IAV) infection of the upper respiratory tract triggers a dynamic immune response that varies both temporally and spatially across nasal tissue compartments. The nasal cavity is composed of functionally distinct regions: the **respiratory mucosa (RM)**, which lines the nasal turbinates and septum and serves as the primary site of viral entry; the **olfactory mucosa (OM)**, a specialized neuroepithelial tissue involved in chemosensation that is particularly vulnerable to infection-associated damage; and the **lateral nasal gland (LNG)**, a secretory gland contributing to mucosal defense. These compartments differ substantially in cell type composition, barrier function, and immune cell residency, making them likely to mount distinct responses to infection.

Prior work has demonstrated that IAV infection of the nasal mucosa drives myeloid cell recruitment by day 5 post-infection (dpi), followed by T cell infiltration peaking around 8 dpi and viral clearance by 14 dpi (Zazhytska et al., 2022; Gallo et al., 2022). However, the tissue-compartment-specific transcriptional dynamics of these immune populations — particularly T cells — remain incompletely characterized at single-cell resolution. This analysis uses scRNA-seq to profile cell type composition and gene expression across all three nasal compartments at five timepoints (naive, D02, D05, D08, D14), focusing on T cell responses during active infection (naive vs. D08).

### Analytical Approach and Tool Justification
...
Stacked bar charts of mean per-mouse cell type proportions at each timepoint revealed dynamic compositional shifts following IAV infection across all three tissues (Figures 5–7). In the RM, neutrophil and immature neutrophil proportions expanded substantially at D05, consistent with the peak myeloid recruitment phase described in the source paper, and then contracted by D08–D14. Most strikingly, and consistent with the T cell infiltration phase annotated in the experimental design (D08), T cell proportions increased visibly at D08 in the RM relative to naive and earlier timepoints. A similar pattern was observed in the OM and LNG, where T cells also rose at D08, though from lower baseline proportions. The LNG showed the highest epithelial cell dominance at naive, with immune proportions remaining relatively modest even at peak infiltration. Given this clear increase in T cell abundance at the D08 timepoint — the timepoint designated as "T cell infiltration" in the source paper — T cells were selected as the focus for differential expression analysis comparing naive and D08.

### Differential Expression in T Cells: Naive vs. D08

Pseudobulk DESeq2 analysis of T cells comparing D08 to naive identified tissue-specific patterns of differential expression (Figure 8; full results in `output_tables/`). In the RM, a large number of genes were significantly differentially expressed (adjusted p < 0.05, |log2FC| > 0.5), with notable upregulation of effector immune genes including Nkg7, Lgals1, Cd3d, and Ly6c2 at D08, as well as the interferon-stimulated gene Gm42418. The OM yielded no significantly upregulated genes at the applied thresholds, and the LNG yielded only Gm42418 as a top hit. This tissue asymmetry suggests the T cell transcriptional response is most pronounced in the RM, the compartment experiencing the highest direct viral burden, while OM and LNG T cells — present in lower numbers — may lack sufficient statistical power to resolve individual gene-level changes under the pseudobulk framework.

Feature plots of T cell subtype markers in the RM D08 subset confirmed co-expression of both Cd4 and Cd8a within the T cell cluster, indicating the presence of both helper and cytotoxic T cell populations (Figure 9). Gzmb and Ifng, markers of cytotoxic effector function, were expressed at notably higher levels than Foxp3 (regulatory T cells) or Tcf7 (stem-like/memory T cells), suggesting that effector rather than regulatory or memory T cells predominate at this timepoint. Violin plots across all six tissue-timepoint conditions confirmed that Gzmb and Ifng are elevated at D08 relative to naive in the RM, and to a lesser degree in the OM and LNG (Figure 10).

### Gene Set Enrichment Analysis

GSEA using GO Biological Process terms and MSigDB Hallmark gene sets revealed strongly tissue-dependent pathway enrichment profiles (Figures 11–16). In the RM, the most significantly enriched activated terms included defense response to virus, response to virus, negative regulation of viral genome replication, chromosome segregation, and nuclear division — consistent with a broad antiviral transcriptional program in infiltrating T cells (Figure 11). Hallmark GSEA in the RM identified enrichment of E2F targets, Interferon Alpha Response, DNA Repair, MYC targets, G2M Checkpoint, Interferon Gamma Response, and mTORC1 signaling as activated gene sets, alongside suppression of metabolic and stromal programs (Figure 14). These results together point to actively proliferating, interferon-stimulated effector T cells in the RM at D08.

In the OM, GO BP GSEA identified only suppressed terms — synaptic translation pathways (translation at presynapse, postsynapse, and synapse) and sensory perception of chemical stimulus — with no significantly activated immune terms (Figure 12). This suppression of neuronal gene sets in T cells within the OM likely reflects contamination of the T cell gene expression profile with olfactory neuron-associated transcripts due to the tissue context, or alternatively a genuine downregulation of these pathways in the local T cell population. Hallmark GSEA in the OM did recover activated immune terms including Interferon Gamma Response, Interferon Alpha Response, E2F Targets, and G2M Checkpoint, alongside suppression of P53 pathway, KRAS signaling, and TNFα signaling via NFκB (Figure 15).

In the LNG, GO BP GSEA revealed exclusively activated terms centered on cell division and mitotic processes — including cell division, spindle organization, mitotic sister chromatid segregation, and negative regulation of mitotic nuclear division — suggesting that the dominant transcriptional signature of D08 T cells in the LNG is one of active proliferation (Figure 13). Hallmark GSEA in the LNG was sparse, identifying only G2M Checkpoint (activated, p.adjust ~0.04) and Oxidative Phosphorylation (weakly activated), consistent with a proliferating but metabolically engaged T cell population (Figure 16).
...
**Figure 2.** UMAP of respiratory mucosa (RM) cells. Top panel: cells colored by annotated cell type. Bottom panel: same UMAP split by timepoint (naive, D02, D05, D08, D14), showing the temporal dynamics of cell type composition in the RM.

**Figure 3.** UMAP of olfactory mucosa (OM) cells. Top panel: cells colored by annotated cell type, highlighting the enrichment of olfactory sensory neuron populations relative to the RM. Bottom panel: UMAP split by timepoint.

**Figure 4.** UMAP of lateral nasal gland (LNG) cells. Top panel: cells colored by annotated cell type, showing the secretory epithelial dominance of this compartment. Bottom panel: UMAP split by timepoint.
...
**Figure 9.** Violin plots of T cell subtype marker expression (Cd4, Cd8a, Foxp3, Tcf7, Gzmb, Ifng) across six tissue-timepoint conditions (RM, OM, and LNG at naive and D08). Gzmb and Ifng are elevated at D08 in the RM, with more modest increases in the OM and LNG.

**Figure 10.** Violin plots of the top significantly upregulated genes in T cells at D08 vs naive in the RM (Gm42418, Ly6c2, Cd3d, Nkg7, Lgals1, Ms4a6b), shown across all six tissue-timepoint conditions. Expression is highest at RM D08, confirming tissue-specific induction.

**Figure 11.** Violin plots of the top significantly upregulated gene in T cells at D08 vs naive in the LNG (Gm42418), shown across all six tissue-timepoint conditions.
...
**Figure 13.** GO Biological Process GSEA dotplot for T cells D08 vs naive in the OM. Only suppressed terms were detected, including synaptic translation (pre-, post-, and synapse), sensory perception of chemical stimulus, and regulation of fat cell differentiation, likely reflecting the neuronal tissue context of the OM.

**Figure 14.** GO Biological Process GSEA dotplot for T cells D08 vs naive in the LNG. All enriched terms are in the activated direction and are centered on mitotic and cell division processes, including cell division, spindle organization, and negative regulation of mitotic nuclear division, consistent with a proliferating T cell population.

**Figure 15.** MSigDB Hallmark GSEA dotplot for T cells D08 vs naive in the RM. Activated sets (left) include E2F Targets, Interferon Alpha Response, DNA Repair, MYC Targets, G2M Checkpoint, Interferon Gamma Response, and mTORC1 Signaling. Suppressed sets (right) include metabolic and stromal programs (Oxidative Phosphorylation, TNFα Signaling, Adipogenesis, Epithelial Mesenchymal Transition).

**Figure 16.** MSigDB Hallmark GSEA dotplot for T cells D08 vs naive in the OM. Activated sets include Interferon Gamma Response, Interferon Alpha Response, E2F Targets, and G2M Checkpoint. Suppressed sets include P53 Pathway, KRAS Signaling Up, Estrogen Response Early, and TNFα Signaling via NFκB.
...
- Gallo O et al. (2022). Innate immunity in the upper airways. *Frontiers in Immunology*, 13. https://doi.org/10.3389/fimmu.2022.841532
...
- Topham DJ et al. (1997). Hematopoietic reconstitution of influenza virus-specific memory T cells. *Journal of Immunology*, 159(3), 1141–1148. https://pubmed.ncbi.nlm.nih.gov/9218337/
...
- Zazhytska M et al. (2022). Non-cell-autonomous disruption of nuclear architecture as a potential cause of COVID-19-induced anosmia. *Cell*, 185(6), 1052–1064. https://doi.org/10.1016/j.cell.2022.01.024
- Zazhytska M et al. (2024). [Source paper for this dataset]. *PMC11324402*. https://pmc.ncbi.nlm.nih.gov/articles/PMC11324402/
